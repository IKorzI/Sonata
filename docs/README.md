...
| EN [English](README.md) | UK [Український](uk/README.md) | RU [Русский](ru/README.md) |
|---|---|---|

Work with documents, saving up to 95% of time with software for professional colleges!

![Preview](https://github.com/IKorzI/Sonata/blob/main/docs/imgs/preview.png)
...
![After launch](https://github.com/IKorzI/Sonata/blob/main/docs/imgs/after_launch.png)
...
* Enabling the screenshot mode occurs in NodeJS after sending a command from Svelte. NodeJS monitors the clipboard and when fixing a range of cells, NodeJS calls a custom file saving window via C++ with the transfer of parameters. After the user consents, C++ returns data to NodeJS. After that, NodeJS, under the control of Microsoft Excel, generates an image and saves it to a temporary directory and sends a command to the Python server to add fields to the image and save it to the directory, which was returned from the C++ save window.
