...
![Превью](imgs/preview1.png)

## Переваги

- Додаток побудовано на базі Electron та Svelte, що забезпечує модульність користувацького інтерфейсу та чітке розділення логіки між окремими компонентами.

![Превью2](imgs/preview2.png)

- Додаток містить декілька тем, що збільшує кастомізацію інтерфейсу.
...
Після запуску потрібно обрати необхідний розділ з меню додатку та працювати з інтерфейсом  
Вигляд інтерфейсу після запуску додатку:
...
| [Додатково](additionally.md)                     | Додаткові модулі додатку                                       |

## Структура додатку

Додаток працює на основі Electron у зв'язці Svelte + NodeJS + Python/C#:

- Після запуску додатку запускається Python-сервер у режимі очікування команд, процес якого закріплюється за процесом Electron
...
![Структура додатку](imgs/schema1.png)
