 ...
 * Після натискання кнопки "Старт" кінцеві дані від користувача переходять через фінальне доповнення та рзрахунки у NodeJS перед відправкою у Python
 ...
 * Увімкнення режиму скріншота відбувається у NodeJS після надсилання команди від Svelte. NodeJS слідкує за буфером обміну і при фіксуванні діапазону клітинок NodeJS викликає кастомне вікно збереження файлу через C++ з передачей параметрів. Після згоди користувача C++ повертає у NodeJS дані. Після цього NodeJS через управління Microsoft Excel формує зображення зі збереженням у тимчасовий каталог і надсилає серверу Python команду для додавання полей у зображення і збереження у каталог, що було повернуто з вікна збереження на C++.

![Структура програми](https://github.com/IKorzI/Sonata/blob/main/docs/imgs/schema1.png)