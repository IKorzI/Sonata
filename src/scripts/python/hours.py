...
def filling_out_the_hours_sheet(
    sheet: Worksheet, month_name: str, year: int, group: object, days: object
):
    """
    ...
    sheet.merge_cells(
        start_row=delete_row_start + 3,
        start_column=3,
        end_row=delete_row_start + 3,
        end_column=38,
    )
    ...
    if delete_col_count > 0:
        ...
        for row in [5, 6, 7, 8, 9, end_row + 4]:
            sheet.unmerge_cells(
                start_row=row, start_column=3, end_row=row, end_column=38
            )
        for col in [37, 38]:
            sheet.unmerge_cells(
                start_row=10, start_column=col, end_row=11, end_column=col
            )
        ...
        sheet.merge_cells(
            start_row=10, start_column=6, end_row=10, end_column=delete_col_start - 1
        )
        for row in [5, 6, 7, 8, 9, end_row + 4]:
            sheet.merge_cells(
                start_row=row,
    ...
    for col in range(6, end_col + 1):
        ...
        sheet.cell(row=end_row + 1, column=col).value = (
            f'=IF(${end_col_plus_1_letter}{end_row + 1}="Очікую","Оч",SUM({col_letter}12:{col_letter}{end_row}))'
        )
    for row in range(12, end_row + 1):
        sheet.cell(row=row, column=end_col + 1).value = (
            f'=IF(SUM(F{row}:{end_col_letter}{row})=0,"Очікую",SUM(F{row}:{end_col_letter}{row}))'
        )
    sheet.cell(row=end_row + 1, column=end_col + 1).value = (
        f'=IF(SUM({end_col_plus_1_letter}12:{end_col_plus_1_letter}{end_row})=0,"Очікую",SUM({end_col_plus_1_letter}12:{end_col_plus_1_letter}{end_row}))'
    )
    ...
    for index, subject in enumerate(group["subjects"]):
        ...
        weekends = [
            val
            for i, val in enumerate(days_list)
            if (i + start_weekday + 1) % 7 in [6, 0]
        ]
...
def filling_out_the_timekeeping_sheet(workbook, months, hours_per_subject):
    ...
    for group_index, group in enumerate(months[0]["groups"]):
        ...
        for row in range(delete_row_start + 3, delete_row_start + 3 + delete_row_count):
            sheet.row_dimensions[row].hidden = True
        sheet.merge_cells(
            start_row=delete_row_start,
            start_column=3,
            end_row=delete_row_start,
            end_column=5,
        )
        ...
        if delete_col_count > 0:
            sheet.delete_cols(idx=delete_col_start, amount=delete_col_count)
            for col in range(
                delete_col_start + 3, delete_col_start + 3 + delete_col_count
            ):
                dims[get_column_letter(col)].hidden = True
        ...
        for row in range(4, end_row + 1):
            sheet.cell(row=row, column=end_col + 1).value = (
                f'=IF(COUNTBLANK(F{row}:{end_col_letter}{row})>0,"Очікую", IF(F{row}-SUM(G{row}:{end_col_letter}{row})=0,"0", IF(F{row}-SUM(G{row}:{end_col_letter}{row})<0,"Надлишок " & (F{row}-SUM(G{row}:{end_col_letter}{row}))*(-1),"Не вистачає " & F{row}-SUM(G{row}:{end_col_letter}{row}))))'
            )
        for col in range(6, end_col + 1):
            col_letter = get_column_letter(col)
            sheet.cell(row=end_row + 1, column=col).value = (
                f'=IF(COUNTBLANK({col_letter}4:{col_letter}{end_row})>0,"Очікую",SUM({col_letter}4:{col_letter}{end_row}))'
            )
        sheet.cell(row=end_row + 1, column=end_col + 1).value = (
            f'=IF(COUNTIF(F{end_row + 1}:{end_col_letter}{end_row + 1},"Очікую")>0,"Очікую", IF(F{end_row + 1}-SUM(G{end_row + 1}:{end_col_letter}{end_row + 1})=0,"0", IF(F{end_row + 1}-SUM(G{end_row + 1}:{end_col_letter}{end_row + 1})<0,"Надлишок "&(F{end_row + 1}-SUM(G{end_row + 1}:{end_col_letter}{end_row + 1}))*(-1),"Не вистачає "&F{end_row + 1}-SUM(G{end_row + 1}:{end_col_letter}{end_row + 1}))))'
        )
        ...
        for index, subject in enumerate(group["subjects"]):
            ...
            sheet.cell(row=row, column=6).value = hours_per_subject[
            ...
            for month in months:
                ...
                sheet.cell(row=row, column=col).value = (
                    f"='[{month_name.capitalize()}.xlsx]{group_code}'!${col_letter}{8 + row}"
                )
                col += 1

        step += 1

    return {"workbook": workbook, "not_found_subjects": not_found_subjects}


def filling_out_the_general_statement_sheet(
    sheet: Worksheet, month: str, year: int, teachers: list, groups: dict
):
    """
    ...
    if delete_col_count > 0:
        for row in [3, 4, 6, 8, end_row + 4]:
            sheet.unmerge_cells(
                start_row=row, start_column=3, end_row=row, end_column=20
            )
        sheet.delete_cols(idx=delete_col_start, amount=delete_col_count)
        ...
        for row in [3, 4, 6, 8, end_row + 4]:
            sheet.merge_cells(
                start_row=row, start_column=3, end_row=row, end_column=end_col + 1
            )
    ...
    for col in range(5, end_col + 1):
        col_letter = get_column_letter(col)
        sheet.cell(row=end_row + 1, column=col).value = (
            f'=IF(SUM({col_letter}10:{col_letter}{end_row})=0,"Очікую",SUM({col_letter}10:{col_letter}{end_row}))'
        )
    for row in range(10, end_row + 1):
        sheet.cell(row=row, column=end_col + 1).value = (
            f'=IF(SUM(E{row}:{end_col_letter}{row})=0,"Очікую",SUM(E{row}:{end_col_letter}{row}))'
        )
    sheet.cell(row=end_row + 1, column=end_col + 1).value = (
        f'=IF(SUM({end_col_plus_1_letter}10:{end_col_plus_1_letter}{end_row})=0,"Очікую",SUM({end_col_plus_1_letter}10:{end_col_plus_1_letter}{end_row}))'
    )
    ...
    for index, teacher in enumerate(teachers):
        ...
        for g_element in teacher["groups"]:
            ...
            if sheet.cell(row=row_for_fill, column=col_for_fill).value is None:
                sheet.cell(row=row_for_fill, column=col_for_fill).value = (
                    f"='{group_name}'!${column_letter}${row}"
                )
            else:
                sheet.cell(
                    row=row_for_fill, column=col_for_fill
                ).value += f" + '{group_name}'!${column_letter}${row}"
...
def hours_BasedOnTheFirstMonth(info, app_path):
    ...
    data = filling_out_the_timekeeping_sheet(
        workbook, info["months"], info["hours_per_subject"]
    )
...
def hours_SummaryOfTeachers(info, appPath):
    ...
    for group in info["groups"]:
        ...
        filling_out_the_hours_sheet(
            sheet, month_name, info["year"], group, info["days"]
        )
        step += 1
    ...
    sheet.title = "Зведена"
    filling_out_the_general_statement_sheet(
        sheet, month_name, info["year"], info["teachers"], groups
    )
