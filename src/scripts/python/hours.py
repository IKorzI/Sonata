...
def filling_out_the_hours_sheet(sheet: Worksheet, month_name: str, year: int, group: object, days: object):
    ...
    if delete_col_count > 0:
        ...
        for _ in range(delete_col_count):
            sheet.delete_cols(delete_col_start)
    ...
    sheet.cell(row=8, column=3).value = cell_C8
...
def filling_out_the_general_statement_sheet(sheet: Worksheet, month: str, year: int, teachers: list, groups: dict):
    ...
    for index, t_element in enumerate(teachers):
        row_for_fill = 10 + index
        sheet.cell(row=row_for_fill, column=4).value = t_element.teacher_name
        for g_element in t_element.groups:
...
def hours_SummaryOfTeachers(info, appPath):
    ...
    sheet.title = "Зведена"
    # Заполняем лист
    filling_out_the_general_statement_sheet(sheet, month_name, info.year, info.teachers_list, groups)
